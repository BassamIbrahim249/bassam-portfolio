...
(function() {
  ...
  // ✅ تعديل: العنوان والنص والكلمات المفتاحية
  {
  ...
  },
  {
    ...
    // ✅ تعديل: النص
    {
      keywords: ['ما هذا الموقع','عن الموقع','موقع ايه','الموقع ده','تعريف بالموقع','ما هو الموقع','نبذة عن الموقع','عن المنصة','المنصة','هدف الموقع','هدف المنصة','غرض الموقع','فكرة الموقع'],      answer: '🌐 <b>منصة BassamIbrahim الرقمية</b> — منصة سودانية متخصصة تهدف إلى نشر العلم والمعرفة في خمسة مجالات رئيسية:<br><br>🏗️ <b>المنصة الهندسية</b> — علوم المواد، ابتكار إنشائي، مختبر هندسي<br>🏛️ <b>الأرشيف السياسي</b> — تاريخ السودان، تحليل استراتيجي، رؤى فكرية<br>🏺 <b>نوبيان</b> — الحضارة النوبية، التاريخ، الآثار، الهوية<br>📚 <b>الأكاديمية</b> — المشاريع، القيادة والإدارة، التدريب التنموي<br>🌿 <b>نمط الحياة</b> — الصحة الشاملة، التغذية، التميز البدني'
    },
    {
  ...
  // ========== دوال معالجة النصوص ==========  function normalize(str) {
  ...
  style.textContent = `
    ...
    #smart-chat-input:focus{border-color:rgba(59,158,255,0.5);}    #smart-chat-send{padding:8px 16px;border-radius:20px;background:#3B9EFF;color:white;border:none;font-family:'Cairo',sans-serif;font-weight:700;font-size:12px;cursor:pointer;transition:background 0.2s;}
  ...
  // ✅ تعديل: تغيير "كتابة المشاريع" إلى "المشاريع"
  ...
  // ✅ تعديل: دالة توليد رد احترافي
  function getProfessionalFallback(question) {
    const q = question.trim();
    
    // أسئلة الدردشة والتعارف
  ...
  async function askHybridExpert(question, articles) {
    ...
    if (articles && articles.length > 0) {
      context = articles.slice(0, 3).map(a => {
        ...
        const content = (a.content_ar || a.content_en || a.content || '').substring(0, 300);        return `عنوان المقال: ${title}\nمحتوى: ${content}`;
      }).join('\n---\n');
    } else {
      context = '';
    }
    ...
    if (!articles.length && !files.length) {
      ...
      const noResultNode = addMsg(`
        ...
        <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;">
          <button class="suggestion-chip ask-general-ai" style="background:#EAB308;color:#000;border:none;font-weight:bold;">🧠 اسأل الخبير العام</button>          <a href="https://wa.me/${WHATSAPP_NUMBER}?text=${enc}" target="_blank" class="whatsapp-link" style="margin-top:0;">💬 تواصل مع بسام</a>
        </div>
