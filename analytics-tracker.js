// analytics-tracker.js — 21 حدثاً استراتيجياً - نسخة مستقرة
(function() {
  'use strict';
  ...
  function attachHandlers() {
    ...
    document.querySelectorAll('button').forEach(function(b) {
      ...
      if (p) {
        b.dataset.t3 = '1';
        b.addEventListener('click', function() { sendEvent('share_' + p); });
      }
    });

    document.querySelectorAll('#smart-assistant-btn').forEach(function(b) {
      if (b.dataset.t4) return;
      b.dataset.t4 = '1';
    ...
    document.querySelectorAll('a[href*="wa.me/249967238251"]').forEach(function(a) {
      if (a.dataset.t5) return;
      a.dataset.t5 = '1';
      a.addEventListener('click', function() { sendEvent('contact_click'); });
    });

    document.querySelectorAll('.search-input').forEach(function(i) {
      if (i.dataset.t6) return;
      i.dataset.t6 = '1';
      var t;
      i.addEventListener('keypress', function(e) {
        if (e.key === 'Enter' && i.value.trim().length >= 2) {
          clearTimeout(t);
          sendEvent('search_articles', { search_query: i.value.trim().slice(0, 100) });
        }
      });
      i.addEventListener('input', function() {
        clearTimeout(t);
        t = setTimeout(function() {
          if (i.value.trim().length >= 2) sendEvent('search_articles', { search_query: i.value.trim().slice(0, 100) });
        }, 1500);
      });
    });

    document.querySelectorAll('a[target="_blank"]').forEach(function(a) {
      if (a.dataset.t7) return;
      var h = a.href || '';
      ...
      a.dataset.t7 = '1';
      a.addEventListener('click', function() { sendEvent('external_link', { target_url: h }); });
    });
  }
  ...
  new MutationObserver(function(m) {
    if (m.some(function(x) { return x.addedNodes.length > 0; })) setTimeout(attachHandlers, 100);
